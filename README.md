...
[![Version](https://img.shields.io/badge/version-2.5.1-E11D48)](https://github.com/RecardoLyu/PerlerBeadsDesigner/releases)
...
本轮把「画」这件事做到了和「转」一样顺手 —— 双端一致的图纸画板，外加参考底图与顺手的选色。

- **图纸画板（双端）**：不依赖照片，直接上手画拼豆图纸。画笔涂抹 / 油漆桶填充 / 橡皮擦 / 框选填充，撤销重做各留 5 步；**真实风 / 图纸风**顶部一键切换（电脑、手机都有入口）。
- **参考底图**：整张外部图像一键铺满画板、**等比例**显示，作为描图参考叠加在绘制层之下。可随时**缩放 / 平移调整**——电脑端滚轮 + 中键拖动、手机端双指捏合 + 拖动，调完「保存调整 / 取消」，只动底图不动画板。
- **色板选色**：实心圆形色块一眼认出真实豆色。手机端把色板**内嵌进图纸画板并置顶**，底部卡片一拉到停顿档就**整块露出**，绘制中随手换色不打断。
- **双端体验统一**：操作命名、底图「不透明度」语义、MARD 色块尺寸等细节双端对齐，换设备不重新学。

**v2.5.1 补丁**

- **生成 / 导出更顺滑**：移动端图纸生成与导出的重计算移入后台线程，点按钮立刻有反馈（按下态 + 颜文字 + 状态灯），不再假死卡顿。
- **更新更稳**：应用内下载 APK 新增完整性校验，被镜像截断 / 污染的坏包自动换源重下，不再报「软件包似乎无效」。

> 当前版本 **v2.5.1**；以上为 v2.5 系列陆续落地的增强与修复，详见「功能特性 · 图纸画板」与 CHANGELOG。
...
- **图纸画板（v2.5 新增）**
  ...
  - **自由手绘拼豆图纸**：画笔涂抹、油漆桶填充、橡皮擦、框选填充，撤销 / 重做各保留 5 步
  - **色板选色**：实心圆形色块对接 5 大品牌真实豆库，画上的就是能买到的真实豆色；手机端色板内嵌进图纸画板，底部卡片一拉到停顿档即整块露出，随手换色
  - 笔触粗细以拼豆板像素为基准，滑块分级调节
  - **参考底图**：整图一键等比例铺满画板、叠加在绘制层之下不干扰作画；可**缩放 / 平移调整**（电脑滚轮+中键、手机双指），保存 / 取消，只动底图不动画板
...
<div align="center">
<sub><b>版本</b> 2.5.1 · <b>最后更新</b> 2026 年 8 月</sub>
</div>
