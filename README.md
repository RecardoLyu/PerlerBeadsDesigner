...
从一张照片到一张可直接对照拼的真实豆子图纸，附带完整物料清单（BOM）。**真实豆子风格**用圆环填色 + 高光还原实物拼豆立体质感，色号与数量统一放在图纸主体之外的 BOM 区，干净易读。
...
- **图纸生成**
  ...
  - 抖动、高阶优化(ICM) 减少噪点与色偏
  - **豆子风格**：真实豆子（圆环填色 + 高光，默认）/ 经典方格（每格印色号），可一键切换
...
- **豆子风格**：真实豆子（圆环 + 高光，贴近实物）/ 经典方格（每格印色号）
- **抖动**：相邻豆交错模拟过渡色
