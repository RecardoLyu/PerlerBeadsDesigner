                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _section(c, Icons.tune_rounded, '调整', [
                      ...
                      _p('裁剪：点「裁剪」后按钮换成较小的「取消 / 应用」。在图上单指拖框选保留区域，拖动边框/四角手柄微调、拖动框内整体移动（带轻微吸附，框外误点不会重新框选）。点「应用」后这张图就成为新的原图（恢复原图只回到裁剪后；想回到最初需重新导入）。'),
                      _p('子交互防护：进入任何新操作（点裁剪、点框选/涂抹、切换底部 Tab）会自动取消上一个未完成的操作。例如裁剪中途点了「开始框选」，裁剪框会立即消失回到浏览态，避免状态串扰。'),
                    ]),
                    ...
                    _section(c, Icons.grid_on_rounded, '图纸（拼豆图）', [
                      ...
                      _p('品牌：选择拼豆品牌颜色库，决定色号与颜色。内置 5 个品牌 —— MARD 曼德(221色，中文名) / Perler(103) / Hama(92) / Artkal S-5mm(199) / Artkal C-2.6mm(174)。切换后旧图纸失效需重新生成，图纸色号严格对应该品牌。'),
                      _p('颜色上限：限制用几种颜色的豆子；0 = 不限制。上限随所选品牌色数自动调整。'),
                      ...
                      _p('图纸顶部信息：左上角始终显示「品牌 · 颜色数 · 总豆数」紧凑单行。文件名标题默认不画在图纸上——在「导出」页勾选「在图纸上显示文件名」后，才把导出文件名居中、粗体画到图纸顶部。'),
                    ]),
                    _section(c, Icons.file_download_rounded, '导出', [
                      _p('生成图纸后到「导出」页，选 1080P / 2K / 4K 分辨率，一键把图纸 PNG 保存到相册的「拼豆图纸」文件夹。'),
                      _p('文件名默认跟随源图文件名；勾选「在图纸上显示文件名」后，该文件名会作为标题画到图纸顶部。'),
                    ]),
                    ...
                    _section(c, Icons.help_outline_rounded, '常见问题', [
                      ...
                      _q(c, '想换豆子品牌 / 色号对不上？', '在「图纸」页「品牌」下拉选你手头的品牌（MARD / Perler / Hama / Artkal S / Artkal C），重新生成即可，色号与 BOM 严格对应该品牌。'),
                      _q(c, '裁剪后「恢复原图」没回到裁剪前？', '这是设计行为：裁剪「应用」后裁剪结果即成为新的原图。想回到最初请重新导入图片。'),
                      _q(c, '裁剪/框选/涂抹中途点了别的按钮？', '软件会自动取消上一个未完成的操作（防护机制）。重新点对应按钮即可重新开始。'),
